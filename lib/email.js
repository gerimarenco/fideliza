...
export async function enviarEmailBienvenida({ email, passwordGenerada, puntosAcreditados, puntosTotales, negocioNombre }) {
  ...
  const negocioNombreSeguro = escaparHtml(negocioNombre)
  ...
  try {
    await resend.emails.send({
      ...
      html: `
        <p>¡Hola!</p>
        <p>Con tu compra en <strong>${negocioNombreSeguro}</strong> arrancaste a sumar puntos en Retornar, su programa de fidelización de clientes — ya sumaste <strong>${puntosAcreditados} puntos</strong>.</p>
...
export async function enviarEmailPuntosAcreditados({ email, puntosAcreditados, puntosTotales, negocioNombre }) {
  ...
  try {
    await resend.emails.send({
      from: EMAIL_FROM,
      to: email,
      subject: `+${puntosAcreditados} puntos en ${negocioNombre}`,
      html: `
        <p>¡Gracias por tu compra en <strong>${escaparHtml(negocioNombre)}</strong>!</p>
