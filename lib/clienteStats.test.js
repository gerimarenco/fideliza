...
test('calcularNivel: umbrales exactos y los bordes de cada nivel', () => {
  ...
  assert.equal(calcularNivel(2999).nombre, 'Bronce')
  assert.equal(calcularNivel(3000).nombre, 'Plata')
  assert.equal(calcularNivel(7999).nombre, 'Plata')
  assert.equal(calcularNivel(8000).nombre, 'Oro')
  assert.equal(calcularNivel(14999).nombre, 'Oro')
  assert.equal(calcularNivel(15000).nombre, 'Diamante')
  assert.equal(calcularNivel(29999).nombre, 'Diamante')
  assert.equal(calcularNivel(30000).nombre, 'VIP')
...
test('descripcionNiveles: de menor a mayor, sin huecos entre rangos', () => {
  ...
  assert.match(lineas[0], /^Bronce: 0 a 2\.999 puntos$/)
  assert.match(lineas[1], /^Plata: 3\.000 a 7\.999 puntos$/)
  assert.match(lineas[2], /^Oro: 8\.000 a 14\.999 puntos$/)
  assert.match(lineas[3], /^Diamante: 15\.000 a 29\.999 puntos$/)
  assert.match(lineas[4], /^VIP: 30\.000\+ puntos$/)
})
